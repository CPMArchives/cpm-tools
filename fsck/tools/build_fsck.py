#!/usr/bin/env python3
"""Build FSCK under CP/M with ZSM4 and Digital Research LINK."""
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="fsck-zsm4-") as temporary:
        work = Path(temporary)
        disks = work / "disks"
        disks.mkdir()
...
expect "B>"
send -- "FSCK C: /VERBOSE\r"
expect "Entry"
expect "Filesystem is consistent."
expect "B>"
send -- "FSCK /H\\r"
expect "FSCK \\[drive:\\]"
expect "B>"
...
expect "B>"
send -- "FSCK /VERSION\\r"
expect "FSCK 0.1.0-dev1"
expect "B>"
send -- "FSCK FOO\\r"
expect "FSCK: unknown option. Use FSCK /HELP."
expect "B>"
send -- "FSCK C: D:\\r"
expect "FSCK: unknown option. Use FSCK /HELP."
expect "B>"
...
'''
        ...
        command_checks = {
            "FSCK C:": "Active entries:",
            "FSCK C: /VERBOSE": "Entry",
            "FSCK /H": "FSCK [drive:]",
            "FSCK /INFO": "Built with ZSM4 and Digital Research LINK.",
            "FSCK /VERSION": "FSCK 0.1.0-dev1",
        }
        ...
        if (result.returncode or "Errors: 0" not in transcript
                or "Active entries:" not in transcript
                or "Filesystem is consistent." not in transcript
        ...
        run("cpmcp", "-f", "ibm-3740", str(disks / "driveb.dsk"),
            "0:FSCK.COM", str(output / "FSCK.COM"))

    digest_com = hashlib.sha256((output / "FSCK.COM").read_bytes()).hexdigest()
    digest_src = hashlib.sha256(SOURCE.read_bytes()).hexdigest()
    (output / "SHA256SUMS.txt").write_text(
        f"{digest_com}  FSCK.COM\n{digest_src}  ../src/FSCK.MAC\n",
        encoding="ascii",
    )
    print(f"FSCK.COM: {(output / 'FSCK.COM').stat().st_size} bytes")
